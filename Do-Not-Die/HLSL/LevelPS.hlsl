...
float4 PS(PS_OUT input) : SV_Target
{
    ...
    final_color += ApplyPointLightLevel(WhiteColor(), input.n, input.origin, input.view_dir);
    final_color += ApplySpotLightLevel(albedo * 3.0f, input.n, input.origin, input.view_dir);
